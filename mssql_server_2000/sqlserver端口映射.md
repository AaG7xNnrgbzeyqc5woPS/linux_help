...
# 11. 总结三(思考和猜想)
  - 服务器vm1,可以使用 1433以外的端口，使用 "VM1\开始\所有程序\Mirosoft SQL Server\服务器端网络适用工具" 修改!
  - ❤️ 服务端口的修改最大的好处是防止 黑客攻击，因为著名端口，众所周知，很容易被攻击！
  - 端口映射为了外部网络访问内部网络而设置的，内部网络访问外部网络一般都不设防，就算是防火墙也是默认内部网络可以访问外部网络的
  - 链接是双向的，有方向的，内部网络到外部网络都会放行的，用路由方式或者使用网络地址转换NAT方式放行。
  - 对于 NAT 外部网络访问内部网络缺省是不通的，需要使用端口映射来解决，所以端口映射都是 从外部网络访问内部网络的
  - 端口映射的设置也都是站在主机位置，以主机 为主角的，就是virtualbox管理系统所在的主机。
  - 主机地址和子系统地址，都主机必须能够访问的地址。就是virtualbox管理系统 能够访问的地址。
  - 映射规则：主机：0.0.0.0:1533, 子系统：127.0.0.1:1433 其实映射到自己的机器上的。
  - ❤️ 应该用子系统的外部地址 10.0.2.15，子系统的内部地址 127.0.0.1,主机是看不到的！
